...
## --------- How does the amount of data affect these distortions? -------------
# summary(DATA$month)
# 
# DAT2<-subset(DATA, month == 1 | month == 2 | month == 3 | 
#                   month == 6 | month == 12 | month == 18)
# 
# summary(as.factor(DAT2$month))
# 
# 
# ## FIM scores with complete data -----------------------------------------------
# g1<-ggplot(DAT2, aes(x = month, y = rasch_FIM)) +
#   geom_point(aes(fill=as.factor(subID)), pch=21, size=2, stroke=1.25) +
#   geom_line(aes(group=subID)) +
#   facet_wrap(~AIS_grade)
# g2<-g1+scale_x_continuous(name = "Time from Admission (Months)", limits=c(0,18)) +
#   scale_y_continuous(name = "Rasch-Scaled FIM Score (0-100)",limits=c(0,100))
# g3 <- g2 + theme_bw() + 
#   theme(axis.text=element_text(size=14, colour="black"), 
#         axis.title=element_text(size=14,face="bold")) +
...
# 
# ## FIM scores with data missing random -----------------------------------------
# g1<-ggplot(DAT2, aes(x = month, y = rasch_FIM_MAR)) +
#   geom_point(aes(fill=as.factor(subID)), pch=21, size=2, stroke=1.25) +
#   geom_line(aes(group=subID)) +
#   facet_wrap(~AIS_grade)
# g2<-g1+scale_x_continuous(name = "Time from Admission (Months)", limits=c(0,18)) +
...
# 
# ## FIM scores with data missing random -----------------------------------------
# g1<-ggplot(DAT2, aes(x = month, y = rasch_FIM_MNAR)) +
#   geom_point(aes(fill=as.factor(subID)), pch=21, size=2, stroke=1.25) +
#   geom_line(aes(group=subID)) +
#   facet_wrap(~AIS_grade)
# g2<-g1+scale_x_continuous(name = "Time from Admission (Months)", limitss=c(0,18)) +
#   scale_y_continuous(name = "Rasch-Scaled FIM Score (0-100)",limits=c(0,100))
# g3 <- g2 + theme_bw() + 
#   theme(axis.text=element_text(size=14, colour="black"), 
#         axis.title=element_text(size=14,face="bold")) +
...
# 
# ## FIM scores with Last Observation Carried Forward ----------------------------
# g1<-ggplot(DAT2, aes(x = month, y = rasch_FIM_LOCF)) +
#   geom_point(aes(fill=as.factor(subID)), pch=21, size=2, stroke=1.25) +
#   geom_line(aes(group=subID)) +
#   facet_wrap(~AIS_grade)
# g2<-g1+scale_x_continuous(name = "Time from Admission (Months)", limits=c(0,18)) +
